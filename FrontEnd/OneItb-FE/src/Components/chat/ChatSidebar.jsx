...
const AvatarBadge = ({ contact, active, unread }) => {
  const avatarUrl = resolveAssetUrl(contact?.avatarUrl);
  const label = `${contact?.firstName || ''} ${contact?.lastName || ''}`.trim() || 'Usuario';
  const fallbackClass = active || unread
    ? 'bg-blue-100 text-blue-700 ring-blue-200 dark:bg-blue-400/20 dark:text-blue-100 dark:ring-blue-300/20'
    : 'bg-slate-100 text-blue-700 ring-slate-200 group-hover:ring-blue-200 dark:bg-white/5 dark:text-blue-200 dark:ring-white/10 dark:group-hover:ring-blue-300/20';
  ...
  return (
    <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold ring-1 ${fallbackClass}`}>
...
export const ChatSidebar = ({
  ...
  const renderContactItem = (contact, isNew = false) => {
    ...
    return (
      ...
      >
        <AvatarBadge contact={contact} active={active} unread={unread} />
