...
export const YouTubeEmbed = ({ videoId, compact = false, tile = false }) => {
  ...
  return (
    <>
      <button
        ...
        className={`group relative isolate flex w-full flex-col items-center justify-center overflow-hidden rounded-lg border border-slate-800 bg-slate-950 text-white shadow-sm ${tile ? 'h-full min-h-0 p-3' : compact ? 'min-h-40 p-4' : 'aspect-video p-6'}`}
...
export const MediaAttachment = ({
  ...
  galleryIndex = 0,
  tile = false,
  landscapeCover = false,
  onIntrinsicImageDimensions,
}) => {
  ...
  if (type === 'image' && !imageFailed) {
    return (
      <>
        <button type="button" onClick={() => setViewerOpen(true)} className={`block w-full overflow-hidden text-left ${tile ? landscapeCover ? 'bg-transparent' : 'h-full bg-slate-100 dark:bg-slate-700/60' : 'rounded-lg'}`}>
          <img
            src={absoluteUrl}
            alt={`Archivo adjunto: ${fileName}`}
            loading="lazy"
            onError={() => setImageFailed(true)}
            onLoad={(event) => onIntrinsicImageDimensions?.(event.currentTarget.naturalWidth, event.currentTarget.naturalHeight)}
            className={tile
              ? landscapeCover
                ? 'h-auto max-h-[30rem] w-full cursor-zoom-in object-contain'
  ...
  if (type === 'pdf' && tile) {
    return (
      <>
        <div className="group flex h-full min-h-36 flex-col overflow-hidden border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-slate-900">
          <div className="min-h-0 flex-1 overflow-hidden">
            <PdfFirstPageThumbnail src={absoluteUrl} title={fileName} compact={compact} fill />
          </div>
          <div className="flex shrink-0 items-center justify-between gap-2 border-t border-slate-200 bg-slate-50 px-3 py-2 dark:border-white/10 dark:bg-slate-800">
            <span className="min-w-0 truncate text-xs font-semibold text-slate-700 dark:text-slate-200" title={fileName}>{fileName}</span>
            <span className="flex shrink-0 gap-1">
              <button type="button" onClick={() => setViewerOpen(true)} aria-label={`Ver ${fileName}`} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-slate-600 ring-1 ring-slate-200 hover:text-blue-600 dark:bg-slate-950 dark:text-slate-200 dark:ring-white/10"><i className="fa-regular fa-eye" /></button>
              <a href={absoluteUrl} target="_blank" rel="noreferrer" download aria-label={`Descargar ${fileName}`} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-blue-600 ring-1 ring-slate-200 hover:bg-blue-50 dark:bg-slate-950 dark:text-blue-300 dark:ring-white/10"><i className="fa-solid fa-download" /></a>
            </span>
          </div>
        </div>
