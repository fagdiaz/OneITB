...
export const ReactionUsersModal = ({ inquiryId, isOpen, onClose }) => {
  ...
  return (
    <div
      ...
      role="presentation"
    >
      <section role="dialog" aria-modal="true" aria-label="Usuarios que dieron Me gusta" className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-slate-950">
        <header className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-white/10">
          <div>
            <h2 className="font-bold text-slate-900 dark:text-white">Me gusta</h2>
        ...
        {page?.hasNextPage && (
          <footer className="border-t border-slate-200 p-3 dark:border-white/10">
            <button type="button" onClick={loadMore} className="w-full rounded-lg bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-100 dark:bg-blue-500/10 dark:text-blue-200 dark:hover:bg-blue-500/15">
              Cargar mas
            </button>
          </footer>
        )}
