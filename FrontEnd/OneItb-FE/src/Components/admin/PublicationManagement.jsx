    ...
    <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Revision operativa del contenido activo del muro.</p>
        </div>
      ...
      {selectedPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{selectedPost.content}</p>
            ...
            {selectedPost.comments?.length > 0 && (
              <div className="mt-5">
                <h4 className="mb-2 text-sm font-bold text-slate-800">Comentarios asociados</h4>
                <div className="space-y-2">
                  {selectedPost.comments.map((comment) => (
                    <div key={comment.id} className="rounded-lg border border-slate-100 bg-white p-3 text-sm">
