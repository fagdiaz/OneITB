    ...
    <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Moderacion rapida de respuestas y conversaciones.</p>
        </div>
      ...
      <div className="space-y-3">
        {comments.map((comment) => (
          <article
            ...
            className="cursor-pointer rounded-xl border border-slate-100 bg-slate-50 p-4 transition hover:border-blue-200 hover:bg-blue-50/40"
          >
          ...
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="text-xl font-bold text-slate-900">Comentario {shortId(selectedComment.id)}</h3>
            ...
            <div className="rounded-xl bg-slate-50 p-4">
              ...
              <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{selectedComment.content}</p>
            </div>
            <div className="mt-4 rounded-xl border border-slate-100 bg-white p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Publicacion relacionada</p>
              <h4 className="mt-2 font-semibold text-slate-900">{selectedComment.postTitle}</h4>
              <p className="mt-1 line-clamp-4 text-sm text-slate-600">{selectedComment.postContent}</p>
            </div>
